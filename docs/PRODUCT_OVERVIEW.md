...
| **Tutor Wallet & Payout** | Weekly automated payouts via Paddle; Vietnam market: MoMo, VNPay, payOS supported as local payout rails; tutor sets their own rates | Tutors won't adopt a new platform if payment is unreliable or opaque |
...
| **Payment infrastructure** | **Paddle** (Merchant of Record — global cards, PayPal, Apple Pay, Google Pay; handles VAT/GST automatically in 200+ countries); MoMo, VNPay, payOS for Vietnam tutor payout rails |
...
| **HIPAA-aligned practices (USA)** | BeQuizzy is not a HIPAA "covered entity" but its Health & Well-being and Mental Health Literacy modules can generate data that constitutes ePHI (electronic Protected Health Information) for US-based users. Required before US Phase 3 launch: (1) BAA-ready infrastructure with cloud providers; (2) strict data minimization on health behavioral data; (3) no use of health data for advertising or AI training without explicit consent; (4) breach notification within 30 days per HIPAA Breach Notification Rule; (5) documented risk assessment (Privacy Impact Assessment). Full HIPAA Business Associate posture required if BeQuizzy integrates with school-based clinics or any US covered entity. |
...
| Payment | Paddle checkout (Visa/Mastercard/PayPal/Google Pay/Apple Pay, priced in VND); MoMo, VNPay, payOS for tutor payout rails; VAT invoice with MST (mã số thuế) auto-generated by Paddle |
...
- International payment stack: **Paddle** as global Merchant of Record (cards, PayPal, Apple Pay, Google Pay; automatic VAT/GST compliance in 200+ countries — no separate tax registration needed per market)
